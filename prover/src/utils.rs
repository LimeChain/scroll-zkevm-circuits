...
pub fn chunk_trace_to_witness_block(mut chunk_trace: ChunkTrace) -> Result<Block<Fr>> {
    if chunk_trace.block_traces.is_empty() {
    ...
    println!("witness_block1 = {:?}", chunk_trace.l1_block_range_hash);
    println!("witness_block2 = {:?}", chunk_trace.l1_block_range_hash.unwrap_or(H256(keccak256(vec![]))));
    panic!("here");
